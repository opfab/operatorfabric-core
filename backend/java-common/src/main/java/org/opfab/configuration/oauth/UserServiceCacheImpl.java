...
package org.opfab.configuration.oauth;
...
public class UserServiceCacheImpl implements UserServiceCache {
    ...
    public UserServiceCacheImpl(
            @Value("${operatorfabric.servicesUrls.users:http://users:2103}") String usersServiceUrl) {
        this.userServiceUrl = usersServiceUrl;

    }

    // The token is stored in the service as when the
    // org.lfenergy.operatorfabric.cards.consultation.services.CardSubscription
    // class call the cache , it does not have the user token
    // it is set each time the user make a request as it can have been refresh in
    // between
