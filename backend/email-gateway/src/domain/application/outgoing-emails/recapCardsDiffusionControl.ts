...
export default class RecapCardsDiffusionControl extends CardsDiffusionControl {
    ...
    public async checkCardsStartingFrom(mode: string): Promise<void> {
        ...
        const cards = (await this.emailGatewayDatabaseService.getCards(dateFrom)) as Card[];

        for (const login of userLogins) {
            try {
                const resp = await this.emailGatewayOpfabServicesInterface.getUserWithPerimetersByLogin(
                    login as string
                );
                if (resp.isValid()) {
