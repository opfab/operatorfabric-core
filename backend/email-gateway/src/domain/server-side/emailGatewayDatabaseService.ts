...
export default class EmailGatewayDatabaseService {
    ...
    public async getSentMail(cardUid: string, email: string): Promise<any> {
        let sent = null;
        try {
            sent = await this.mongoDB.collection('emailGateway-emailsAlreadySent').findOne({cardUid, email});
        } catch (error) {
    ...
    public async persistSentMail(cardUid: string, email: string): Promise<void> {
        try {
            await this.mongoDB
                .collection('emailGateway-emailsAlreadySent')
    ...
    public async deleteMailsSentBefore(limitDate: number): Promise<void> {
        try {
            await this.mongoDB.collection('emailGateway-emailsAlreadySent').deleteMany({date: {$lte: limitDate}});
        } catch (error) {
            this.logger.error('Mongo error saving sent mail' + JSON.stringify(error));
        }
    }
