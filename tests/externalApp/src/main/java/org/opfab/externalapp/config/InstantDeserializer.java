    ...
    public Instant deserialize(JsonParser parser, DeserializationContext ctxt) throws IOException {
        ...
        switch (parser.currentTokenId()) {
            case JsonTokenId.ID_START_OBJECT:
                ...
                if (node == null)
                    throw new IOException("Impossible to deserialize Instant object");
                return Instant.ofEpochSecond(Long.parseLong(node.get("epochSecond").toString()),
                        Long.parseLong(node.get("nano").toString()));
