Feature: uploadUIMenu
  ...
  Scenario: Check that UIMenu has been updated
    ...
    # Check usercard icon restriction
    * def userCard = response.topRightIconMenus.filter(x => x.opfabCoreMenuId == 'usercard')[0]
    * match userCard.showOnlyForGroups ==
      """
      ['ADMIN', 'Dispatcher', 'Planner', 'Supervisor', 'Manager', 'Maintainer']
      """

    # Calendar intentionally has no restriction
    * def calendar = response.topRightIconMenus.filter(x => x.opfabCoreMenuId == 'calendar')[0]
    * match calendar.showOnlyForGroups == '#notpresent'