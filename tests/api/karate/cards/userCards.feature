...
#get card with user operator1_fr
    ...
    And param cardId = 'api_test.initialCardProcess'
    And header Authorization = 'Bearer ' + authTokenAsTSO
    When method get
    Then status 200
...
#get card with user operator1_fr
    Given url opfabUrl + 'cards-consultation/cards'
    And param cardId = 'process_1.process_id_w'
    And header Authorization = 'Bearer ' + authTokenAsTSO
    When method get
    Then status 200
...
# We check externalApp has sent a card following reception of card process_1.process_id_w
    * configure retry = { count: 3, interval: 3000 }
    Given url opfabUrl + 'cards-consultation/cards'
    And param cardId = 'api_test.process_id_w_created'
...
# verify that child card was not deleted after parent card update
    Given url opfabUrl + 'cards-consultation/cards'
    And param cardId = 'process_2.process_o'
    And header Authorization = 'Bearer ' + authTokenAsTSO
    When method get
    Then status 200
...
# We check that the child card exists (childCard1)
    Given url opfabUrl + 'cards-consultation/cards'
    And param cardId = 'process_1.process_id_4'
    And header Authorization = 'Bearer ' + authTokenAsTSO
    When method get
    Then status 200
    And match response.card.parentCardId == cardId
...
# We check that the child card exists (childCard2)
    Given url opfabUrl + 'cards-consultation/cards'
    And param cardId = 'process_1.process_id_5'
    And header Authorization = 'Bearer ' + authTokenAsTSO
    When method get
    Then status 200
    And match response.card.parentCardId == cardId
