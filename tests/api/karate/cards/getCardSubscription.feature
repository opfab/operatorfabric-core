...
#Attach perimeter to group
    ...
    * def userSettings =
    ...
    # Get subscription with processStatesToSendEvenIfNotNotified set to api_test.messageState and check that card is returned
    Given url opfabUrl + 'cards-consultation/cardSubscription' +'?clientId=ghi0123456789jkl&processStatesToSendEvenIfNotNotified=api_test.messageState&updatedFrom=0'
      And header Authorization = 'Bearer ' + authTokenAsTSO
      When method get
      Then status 200
      And match response contains '"card":{'
