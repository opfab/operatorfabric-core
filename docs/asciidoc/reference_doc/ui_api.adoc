...
Obtain the login of the current user :

```
const login = opfab.users.currentUser.getLogin();
```

==== Function getFirstName

Obtain the first name of the current user :

```
const firstName = opfab.users.currentUser.getFirstName();
```

==== Function getLastName

Obtain the last name of the current user :

```
const lastName = opfab.users.currentUser.getLastName();
```

==== Function getGroups

Obtain the list of group ids the current user belongs to :

```
const groups = opfab.users.currentUser.getGroups();
```

The function returns an array of group id strings.

==== Function getEntities

Obtain the list of entity ids the current user belongs to :

```
const entities = opfab.users.currentUser.getEntities();
```

The function returns an array of entity id strings.
...
It is possible to get business data json file content: 
```
const businessData = opfab.businessconfig.businessData.get('businessDataFileName');
```

The function take as input the name of the business data file (without the extension) and returns the content of the json file as a javascript object.


==== Function registerFunctionToListenToBusinessDataUpdates

It is possible to register at startup the function to be used to listen to business data updates:
```
...
The function given as parameter will be called by opfab when business data need to be refreshed (i.e a business data file has changed on server side , it is not possible to specify which file has changed).

[WARNING]
====
It is not yet possible to unregister the function once registered so you should avoid registering function multiple times (it must be avoided in templates for example).
====


=== Function registerCustomScreen 

It is possible to register at startup a custom screen definition: 

```
opfab.businessconfig.registerCustomScreen(screenDefinition);
```

See chapter <<card_list_custom_screen, custom screens>> for more details on the screen definition structure.
...
https://github.com/opfab/operatorfabric-core/blob/{gitReference}/config/docker/externalJs/loadTags.js[config/docker/externalJs/loadTags.js].
...
An example of usage can be found in the file
https://github.com/opfab/operatorfabric-core/blob/{gitReference}/config/docker/externalJs/handlebarsExample.js[config/docker/externalJs/handlebarsExample.js].
...
It’s possible to redirect the user from a card to a business application declared in the ui menu configuration.
...
- idMenu is the id of the menu entry defined in the ui menu configuration
...
To check if the current user has the permission to edit the current card, you can call the isUserAllowedToEdit() method.
```
...
It is possible to open current card edition modal, if the current user has the permission, by calling the editCard() method.
```
...
To know if the template is locked (i.e. user can not respond unless he unlocks the card)
```
...
An example of usage can be found in the file
https://github.com/opfab/operatorfabric-core/blob/{gitReference}/tests/resources/bundles/conferenceAndITIncidentExample/template/incidentInProgress.handlebars[tests/resources/bundles/conferenceAndITIncidentExample/template/incidentInProgress.handlebars].
...
The template can know if the current user is a member of an Entity required to respond by calling the isUserMemberOfAnEntityRequiredToRespond function.
An example of usage can be found in the file https://github.com/opfab/operatorfabric-core/blob/{gitReference}/tests/resources/bundles/defaultProcess_V1/template/question.handlebars[tests/resources/bundles/defaultProcess_V1/template/question.handlebars].
...
Register a function to listen for template destruction (i.e., when a card is removed from display). This is especially useful if you need to remove JavaScript event listeners.
...
=== Function listenToResponseFromChildcardsSending

Register a function that will be called after a child card (or response card) has been sent. The function will receive as parameter an object with the following fields:
 - error: boolean indicating if an error occurred during the sending of the child card
 - message: in case of error, the error message 
 - publisher: the entity on behalf of which the child card has been sent

``` 
    opfab.currentCard.listenToResponseFromChildcardsSending((response) => {
         console.log("Response from child card sending: ", response);
    });
```
...
Register a function to be informed when the template is locked (i.e. user has responded to the current card)
...
Register the template function to call to get user response. This function will be called by opfab when the user clicks on the "send response" button. More explanation can be found in the <<response_cards, response card chapter>>.
...
Register a function to listen for template destruction (i.e., when a usercard is removed from display). This is especially useful if you need to remove JavaScript event listeners.
...
You can use this function to get a list of cards filtered on the specified filter. The cards will be retrieved via a request to the backend.
...
A light card is a card that does not contain the fields present in "data", except for those fields used in custom screens and declared in `common.yml` (`operatorfabric.customscreen.dataFields`) in the backend configuration. The advantage is that light cards are kept in memory and do not require backend access, unlike the Card API.

=== Function getCurrentUserChildCardsForParentCard

You can use this function to get the child cards of the current user's entities for a given parent card.


```
const childCards = opfab.lightCards.getCurrentUserChildCardsForParentCard(parentCardId);

```
The function returns an array of child cards.

The `parentCardId` parameter is the id of the card for which you want to retrieve the child cards.
