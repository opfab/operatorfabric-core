...
- The field "showConfirmationPopup" is used to display a confirmation popup before sending the response. The default value is false.
...
[NOTE]
====
There is also the possibility to replace the "validate answer" and  "modify answer" default button labels in the config.json of the process: 
....
  "questionState": {
    "validateAnswerButtonLabel": "Submit your answer",
    "modifyAnswerButtonLabel": "Change your answer"
....
====
...
`config.json` will be used for the response.
    ...
    * severity (_string_) : an optional field used to define the severity of the response card. The default value is "INFORMATION".
...
In addition, if the user choses the refusal option, it overrides the response state defined in the `config.json` with another
state. It is also possible to dynamically set the severity of the response with a logic from the template. Here the severity of the response depends on the number of choices selected by the user. 

.tests/resources/bundles/defaultProcess_V1/template/question.handlebars
[source,js]
----
include::../../../tests/resources/bundles/defaultProcess_V1/template/question.handlebars[tag=functionToGetUserResponse_example]
include::../../../tests/resources/bundles/defaultProcess_V1/template/question.handlebars[tag=opfab.currentCard.registerFunctionToGetUserResponse_example]
----


[NOTE]
====
The method registered via `opfab.currentCard.registerFunctionToGetUserResponse` can also return a Promise of the object described above instead of the object itself, to allow asynchronous processing in the template before sending the response.
====
...
The defined method shall have an array of child cards as a parameter and will be called by OperatorFabric when loading the card and every time the list of child cards changes.
...
You can find an example in the file https://github.com/opfab/operatorfabric-core/blob/{gitReference}/tests/resources/bundles/defaultProcess_V1/template/question.handlebars[tests/resources/bundles/defaultProcess_V1/template/question.handlebars].
