...
Users:: represent account information for a person intended to receive cards in the OperatorFabric instance.
Entities::
- represent set of users intended to receive collectively some cards.
...
NOTE: When a user is created (either manually via API or automatically via authentication), a `creationDate` field is automatically set with the current date and time. This field is read-only and preserved during updates. It is displayed in the admin users screen and can be retrieved via the users API.
...
entity is stored in the user's data (`entities` field of the `user` object). 

In Entity objects, the `parents` property (array) expresses the fact that this entity is a part of one or several other entities. This feature allows cards to be sent to a group of entities without having to repeat the detailed list of entities for each card.

The roles attribute allows to set the utility of an entity. It determines if an entity can send or receive cards and if it's part of an activity area. This can be useful for a group of entities where only child entities should be used as card publishers while the parent is just a logical group usable as a card recipient.

The `description` property is used, in addition to the label, for searching in the multi-select on the user card screen (recipient and recipient for information fields).

The `labels` property (array) allows to associate string labels to an Entity object.
...
The available permissions are:
 ...
 - `VIEW_ALL_CARDS`: Permission to access all cards and archived cards
...
If set visible in the ui menu configuration, this interface is accessible via the user menu (top right of the screen).

The choice of activity area may be done during user logging phase if you set activityArea.selectActivityAreaOnLogin to true in web-ui.json.
...
If set visible in the ui menu configuration and user is admin, this interface is accessible via the user menu (top right of the screen).
