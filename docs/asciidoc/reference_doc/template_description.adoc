 ...
 * a *card* property (See
ifdef::single-page-doc[link:../api/index.html#tag/cards-publication[card publication API]]
ifndef::single-page-doc[link:/documentation/current/api/index.html#tag/cards-publication[card publication API]]
...
Your can find example using these classes in the OperatorFabric core repository (https://github.com/opfab/operatorfabric-core/blob/{gitReference}/tests/resources/bundles[tests/resources/bundles]).
...
The library https://www.chartjs.org/[charts.js] is integrated in OperatorFabric, it means it's possible to show charts in cards, you can find a bundle example in the operator fabric git (https://github.com/opfab/operatorfabric-core/blob/{gitReference}/tests/resources/bundles/defaultProcess_V1[tests/resources/bundle/defaultProcess_V1]).
