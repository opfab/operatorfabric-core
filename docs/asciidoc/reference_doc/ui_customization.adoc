...
It is possible to see cards in custom screens even if cards are filtered by the user via the notification configuration screen. To do so, you can configure in the web-ui.json the list of processes/states for which the card should be visible even if it is filtered. The parameter to configure is `processStatesLoadedRegardlessOfNotificationConfig` and the value is an array of process/state in the format "processName.stateName".
...
The bubbles with the number of cards per severity are still displayed on the dashboard tiles, but they take into account the list of card that will be displayed in the custom screen linked to the cell instead of the total number of cards of the process/state.
