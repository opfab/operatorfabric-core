...
NOTE: The custom screen IDs referenced in `processCustomLinks` must match the IDs of custom screens registered via `opfab.businessconfig.registerCustomScreen` or custom menu IDs defined in the UI menu configuration.
...
Each cell has the same structure as `processCustomLinks`:
