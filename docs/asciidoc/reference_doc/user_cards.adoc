...
core repository under (https://github.com/opfab/operatorfabric-core/blob/{gitReference}/tests/resources/bundles/conferenceAndITIncidentExample[tests/resources/bundles/conferenceAndITIncidentExample]).
...
https://github.com/opfab/operatorfabric-core/blob/{gitReference}/tests/resources/bundles/taskExample/template/usercard_task.handlebars[tests/resources/bundles/taskExample/template/usercard_task.handlebars].
...
[NOTE]
====
The method registered via `opfab.currentCard.registerFunctionToGetSpecificCardInformation` can also return a Promise of the object described above instead of the object itself, to allow asynchronous processing in the template before sending the card.
====
...
== send a card on behalf of the user

By default, when the user sends a card via the usercard interface, the card is sent on behalf of the entity of the user.
If you want to send a card on behalf of the user himself, you have to set the field `publisherType` to `USER` in the
usercard template, via the function `registerFunctionToGetSpecificCardInformation`. This way, the user will be set as
...
false in the state configuration, in the file `config.json` of the corresponding bundle.
