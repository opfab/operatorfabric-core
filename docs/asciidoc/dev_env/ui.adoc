...
Within the folder `${OF_HOME}/frontend`, run `ng build` to build the project.

The build artifacts will be stored in:

[source,shell]
----
${OF_HOME}/frontend/build/distribution
----
...
Run in the `${OF_HOME}/frontend` directory the command `ng test --watch=false` to execute the unit tests
...
. start opfab in dev mode: `${OF_HOME}/bin/startOpfab.sh`;
...
If it is still not running , launch in the frontend directory 
