...
`$OF_HOME/bin/startOpfab.sh` script.

As described in `$OF_HOME/config/docker-compose.yml`, in addition to lauching all OperatorFabric services it
...
.$OF_HOME/config/services/external-devices-docker.yml

[source,yaml]
----
include::../../../../config/services/external-devices-docker.yml[]
----
...
----
$OF_HOME/bin/startOpfab.sh
----

NOTE: Configuration for the dummy modbus device can be provided in
`tests/dummyModbusDevice/src/main/resources/application.yml`.
...
See the https://github.com/opfab/operatorfabric-core/blob/develop/tests/dummyModbusDevice/README.adoc[README for the Dummy Modbus Device module].
