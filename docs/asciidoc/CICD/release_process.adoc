...
----
+
. Use the ./bin/prepare_release_version.sh script to automatically perform all the necessary changes:
+
----
./bin/prepare_release_version.sh -v X.X.X-RC.RELEASE
----
+
...
docs/asciidoc/resources/index.adoc and include a link to it at the top of the release notes.
...
NOTE: The `new_api_documentation` property should be set to true for all new versions, so the new API documentation is displayed on the website.
...
+
----
./bin/prepare_release_version.sh -v X.X.X.RELEASE
----
+
You should get the following output:
+
...
The following files have been updated:
 M VERSION
 M config/docker-compose.yml
 M src/doc/api/openapi.yml
----
+
This script performs the following changes:
+
