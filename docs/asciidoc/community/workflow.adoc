...
. Run a `docker compose` with the `${OF_HOME}/tests/test-environment/docker-compose.yml` file;
...
. Go to `${OF_HOME}/tests/resources/`;
. Run the following scripts: `./loadTestConf.sh &&  ./send6TestCards.sh`;
. Open the front-end in a browser and look for any regression.

To automate build and API testing, you can use `${OF_HOME}/tests/api/karate/buildAndLaunchAll.sh`. 
