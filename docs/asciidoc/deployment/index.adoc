...
It is also possible to monitor the health status of email-gateway, cards-reminder and supervisor services by making an HTTP GET request to the "/healthcheck" endpoint exposed by each service. The healthcheck endpoint will return a HTTP 200 status code if the service is up and running.
...
Log levels can be defined either globally in the YAML configuration file or individually for each service. You will find a commented configuration for each service within the directory config/services.
...
==== Node Services
For node services (supervisor, email-gateway and cards-reminder) the available logging levels are:
...
ifndef::single-page-doc[<</documentation/current/deployment/index.adoc#cardSendingLimitCardCount, `cardSendingLimitCardCount`>>] 
in a period of time.
ifdef::single-page-doc[<<cardSendingLimitPeriod, `cardSendingLimitPeriod`>>]
ifndef::single-page-doc[<</documentation/current/deployment/index.adoc#cardSendingLimitPeriod, `cardSendingLimitPeriod`>>]
.
...
ifndef::single-page-doc[<</documentation/current/deployment/index.adoc#activateCardSendingLimiter, `activateCardSendingLimiter`>>]
.
...
== Manual purge of cards (administration)

OperatorFabric includes an automatic purge mechanism that periodically removes old cards and archived cards according to configured retention policies.

It is also possible to trigger this purge manually.

=== Purge behavior

The purge logic is based on the card `publishDate`.

Two retention strategies are applied:

* cards without `endDate` use:
** `operatorfabric.cards-publication.purge.defaultDaysToKeepCardsAfterPublication`
** `operatorfabric.cards-publication.purge.defaultDaysToKeepArchivesCardsAfterPublication`

* cards with `endDate` use:
** `operatorfabric.cards-publication.purge.defaultDaysToKeepCardsAfterEndDate`
** `operatorfabric.cards-publication.purge.defaultDaysToKeepArchivesCardsAfterEndDate`

For cards with an `endDate`, both conditions must be met for the purge to occur:

* the `publishDate` retention limit must be reached
* the `endDate` retention limit must be reached

=== Using the REST API

The purge can be triggered via the following endpoint:

----
POST "$url:2102/cards/purge" -H "Authorization:Bearer $token"
----

This endpoint executes the same purge logic as the scheduled job by calling the internal CardPurgeService.

It will:

* delete old cards according to the configured retention policies
* delete archived cards according to the configured retention policies
* update archived cards deletion dates when applicable
* apply only if `operatorfabric.cards-publication.purge.activate=true`

NOTE: This operation is restricted to administrators.

=== Using the CLI

The purge can also be triggered using the OperatorFabric CLI:

----
opfab card purge
----

This command internally calls the same REST endpoint and provides a simpler way for administrators to trigger the purge without manually constructing HTTP requests.
