...
We can add him in our OperatorFabric instance using the following command, using cURL:
...
NOTE: When a user is created, a `creationDate` field is automatically set with the current date and time (epoch time in milliseconds). This field is read-only and cannot be modified.
...
To correct this mistake, the following command can be used, using cURL:
....
