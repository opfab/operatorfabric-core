...
* `config/common.yml` the common configuration file for back services
...
- `security.logout-url`: url used when a user is logged out of the UI (not used for CODE and IMPLICIT flows);
...
This has to be defined in web-ui.json by setting : 

- `security.oauth2.flow.mode` to `PASSWORD`;
- `security.oauth2.flow.delegate-url` with the URL of the OAuth2 leading to the protocol used for authentication.

==== Example of Configuration For PASSWORD Flow
...
=== OAuth2 CODE and IMPLICIT Flows

To enable IMPLICIT or CODE Flow authentication the following properties need to be set in web-ui.json:

- `security.oauth2.flow.mode` to `IMPLICIT` or `CODE`;
...
It is possible to configure the URL to redirect the browser after logout by setting the `security.post-logout-url` parameter in web-ui.json.
...
----
{
  "security": {
    ...
    "post-logout-url": "https://opfab.github.io"
  }
}
----
...
For CODE mode, it is possible to set a silent refresh by setting `security.oauth2.useSilentRefresh` to true. For IMPLICIT mode, silent refresh is always enabled and not configurable.
...
|operatorfabric.security.jwt.groups.rolesClaim.rolesClaimStandardList.path||no| path in the JWT to retrieve the claim that defines a list of groups
|operatorfabric.security.jwt.groups.rolesClaim.rolesClaimStandardList.separator||no| set the separator value of the list of groups
...
|operatorfabric.security.jwt.entitiesIdClaimSingleString|false|no| a boolean specifying the expected format of the entitiesId field in the token. +
- true: a single string with all entities, separated by a semicolon +
- false: an array of strings, each one is an entity
|operatorfabric.security.jwt.gettingEntitiesFromToken|false|no| boolean indicating if you want the entities of the user to come from the token and not mongoDB (possible values: true/false)
|operatorfabric.security.jwt.gettingFirstAndLastNameFromToken|false|no| boolean indicating if you want the first name and last name of the user to come from the token and not mongoDB (possible values: true/false)
|===
