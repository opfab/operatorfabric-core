...
The configuration is centralized in the *config* directory of the GitHub repository. This directory contains a README.md file that explains its contents.
...
OperatorFabric is supporting mongo version 5.0.0 and above.
...
Examples of configuration of each service can be found under config/services. 
...
|operatorfabric.users.getEmailFromUserInsteadOfSettings|false| If set to true, the email address used to notify the user
is fetched from the token, otherwise the user must fill the field on the settings screen
...
OperatorFabric is also able to publish response cards to a Kafka topic. The default topic name  `opfab-response`. You can specify which response cards
...
The email gateway service can be configured with the following properties:
...
|operatorfabric.emailGateway.imapServer.authTimeout||yes|Defines the maximum amount of time (in milliseconds) the client will wait for the authentication process to complete before timing out
|operatorfabric.emailGateway.imapServer.host||yes|Mail server host
|operatorfabric.emailGateway.imapServer.port||yes|Mail server port
|operatorfabric.emailGateway.imapServer.tls||yes|Flag to set whether the connection to the IMAP server should be secured using TLS (Transport Layer Security)
...
|operatorfabric.emailGateway.defaultConfig.incomingEmails.mailboxes||yes|List of mailboxes that the email gateway will monitor
|operatorfabric.emailGateway.defaultConfig.incomingEmails.mailboxes[].mailbox||yes|Email address of the mailbox to monitor
|operatorfabric.emailGateway.defaultConfig.incomingEmails.mailboxes[].password||yes|Password used to authenticate to the mailbox via IMAP
|operatorfabric.emailGateway.defaultConfig.incomingEmails.mailboxes[].emailToCardConverter||yes|Identifier of the converter used to transform incoming emails into OperatorFabric cards
|operatorfabric.emailGateway.defaultConfig.incomingEmails.secondsBetweenConnectionChecks|10|no|Connection check period
