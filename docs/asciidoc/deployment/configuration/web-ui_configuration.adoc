...
|loadNotNotifiedLightCard|false|no|If set to true, light cards are loaded in the UI even if user has set the card as not notified in the notification configuration screen (cards are still not visible on the feed) .This is to be used only if you the dashboard screen to show not notified cards. Be aware that this can lead to performance issues if too much cards.
