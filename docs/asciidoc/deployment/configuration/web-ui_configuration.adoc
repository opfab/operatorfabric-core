...
----
|activityArea.hideConnectedUsers|false|no|If set to true, connected users are not displayed in the activity area screen
|activityArea.selectActivityAreaOnLogin|false|no| if set to true the users belonging to multiple Entities will be required to configure activity area on login
...
|feed.enableGroupedCards|false|no|: If true cards with the same tags are grouped together. Clicking on the grouped card will show the other cards with the same tags in the feed.
|feed.showSearchFilter|false|no| If set to false, the search filter is hidden. The search filter applies to the title and summary of the cards.
...
|processStatesToSendEvenIfNotNotified|[]|no|Array of strings specifying process.state pairs (e.g., ["process1.state1", "process2.state2"]) that should be sent to the UI even if the user has configured them as not notified in the notification configuration screen. These cards will still not be visible in the feed but will be available for other screens (e.g., dashboard). Each entry should follow the format "processId.stateId". Be aware that this can lead to performance issues if too many process/state pairs are configured.
...
a|Url to redirect the browser for authentication. Mandatory with CODE and IMPLICIT flows and must be the url part before `.well-known/openid-configuration` (for example in dev configuration it's `http://localhost:89/auth/realms/dev`).
...
|security.oauth2.useSilentRefresh|false|no| Set to true for using silent refresh with CODE flow (for IMPLICIT flow silent refresh is always enabled and not configurable) 
|security.post-logout-url|the opfab url|no| The redirect URL after logout when using IMPLICIT and CODE flow 
...
|feed.geomap.colorMode|uiTheme|no| Controls **how/if** the night filter is applied. `"dark"` = always dark (apply filter), `"light"` = always light (no filter), `"uiTheme"` = follows application theme.
...
**Examples:**
```jsonc
// Always follows application theme (default)
"colorMode": "uiTheme"

// Always dark map with inverted tiles
"colorMode": "dark"

// Always light map without filter
"colorMode": "light"
```

The `"dark"` value applies CSS inversion filter while `"light"` ensures no filter is applied, regardless of the application theme.
...
All layer types support the following optional properties:
    ...
    - `zIndex`: Integer between 0 to 1999 (default: 0). Controls layer stacking order. Higher values appear on top.
