...
|processStatesAlwaysLoaded|[]|no|Array of strings specifying process.state pairs (e.g., ["process1.state1", "process2.state2"]) that should be sent to the UI even if the user has configured them as not notified in the notification configuration screen. These cards will still not be visible in the feed but will be available in card list custom screens. Each entry should follow the format "processId.stateId". Be aware that this can lead to performance issues if too many process/state pairs are configured.
