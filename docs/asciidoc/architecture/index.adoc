...
Business components are based on https://spring.io/projects/spring-boot[SpringBoot] or https://nodejs.org[Node.js] and are packaged via https://www.docker.com/[Docker]. 
...
The main idea is to keep business logic as independent from technology as possible. For example, business code should not depend on the Angular framework. Inspired by hexagonal architecture, this results in the following simplified view:

image::SoftwareArchitecture-UI.png[back software architecture diagram]

Rendering components should contain minimal business logic. Each component is split into two parts: a component part (the Angular component) and a "View" part (e.g., `view/myComponentView.ts`) that contains business logic specific to the component.

Code inside the services directory contains business logic shared between components or logic not directly related to the UI.

The API component acts as a gateway to access business methods and should not contain any business logic.

Unit testing focuses on testing business logic, using mock objects to simulate servers (see `app/tests/mocks`).
