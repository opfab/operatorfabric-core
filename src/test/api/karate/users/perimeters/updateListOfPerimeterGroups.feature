Feature: Update list of perimeter groups (endpoint tested : PUT /perimeters/{id}/groups)
  ...
  Background:
    ...
    * def authTokenAsTSO = signInAsTSO.authToken
       ...
       #defining perimeters
    * def perimeter = 'perimeterKarate4_1'

    * def perimeterKarate4_1 =
"""
{
   "id" : "perimeterKarate4_1",
   "process" : "process4",
   "state" : "state1",
   "rights" : "Read"
}
...
"""
  ...
  #Create new perimeter (check if the perimeter already exists otherwise it will return 200)
    ...
    And request perimeterKarate4_1
    When method post
    Then status 201
    And match response.id == perimeterKarate4_1.id
    And match response.process == perimeterKarate4_1.process
    And match response.state == perimeterKarate4_1.state
    And match response.rights == perimeterKarate4_1.rights
  ...
  Scenario: Check that groupKarate9 has the perimeter
    ...
    When method get
    Then status 200
    And match response[0].id == perimeterKarate4_1.id
    And match response[0].process == perimeterKarate4_1.process
    And match response[0].state == perimeterKarate4_1.state
    And match response[0].rights == perimeterKarate4_1.rights


  Scenario: Update list of perimeter groups without authentication
    Given url opfabUrl + 'users/perimeters/'+ perimeter + '/groups'
    And request groupList
    When method put
    Then status 401
