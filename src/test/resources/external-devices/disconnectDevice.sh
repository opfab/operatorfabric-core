...
if [ -z $1 ]
then
    echo "Usage : disconnectDevice.sh deviceId opfab_url"
else
	source ../getToken.sh "admin" $url
    curl -s -v -X POST "$url:2105/devices/$1/disconnect" -H "Content-type:application/json" -H "Authorization:Bearer $token" 
	echo ""
fi
