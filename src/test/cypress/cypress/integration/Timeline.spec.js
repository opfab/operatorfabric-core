...
describe('Time line moves', function () {
    ...
    before('Set up configuration and cards', function () {
        ...
        script.setPropertyInConf('heartbeatSendingInterval', 360000);
        script.setPropertyInConf('settings.locale', '\\"en\\"');
        script.setPropertyInConf('settings.weekStartsOn', 6);
    });
    ...
    it('Check timeline moves when unlocked in real time view', function () {
        ...
        script.sendCard('cypress/feed/customEvent.json', {
            startDate: deltaTime - (2 * HOURS + 15 * MINUTES),
    ...
    it('Check timeline moves when unlocked in day view', function () {
        ...
        script.sendCard('cypress/feed/customAlarm.json', {
            startDate: deltaTime + 5 * HOURS,
            endDate: deltaTime + 6 * HOURS
        });
        cy.wait(500);
        cy.tick(1 * SECONDS);
        checkHaveCircle(1);
    ...
    it('Check timeline moves when unlocked in week day view', function () {
        ...
        deltaTime = currentDate.getTime() - new Date().getTime();
        script.sendCard('cypress/feed/customAlarm.json', {
            startDate: deltaTime + 72 * HOURS,
            endDate: deltaTime + 80 * HOURS
        });
        cy.wait(500);
        cy.tick(1 * SECONDS);
        checkHaveCircle(1);
    ...
    it(
        'Check timeline circles have valid popover',
        {
            retries: {
