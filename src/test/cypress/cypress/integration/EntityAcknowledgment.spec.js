...
describe('Entity acknowledgment tests for icon in light-card', function () {
    ...
    it(
        'operator4_fr acknowledges the 2 cards, we check the icons, then he un-acknowledges the 2 cards ' +
            'and we check again the icons',
        function () {
    ...
    it(
        '1) operator1_fr acknowledges the card "message2", and we check the icon is not present for operator4_fr\n' +
    ...
    it('check entities ack for cards sent to groups only (no entityRecipients) for operator2_fr', function () {
        ...
        cy.get('#cardUid').then(($cardUidElement) => {
            ...
            script.sendAckForCard(
                'operator4_fr',
                cardUid,
                '[\\"ENTITY1_FR\\",\\"ENTITY2_FR\\",\\"ENTITY3_FR\\",\\"ENTITY4_FR\\"]'
            );
            cy.waitDefaultTime();
            cy.get('#opfab-feed-light-card-cypress-entitiesAcksMessage2_groupsOnly .fa-check').should('not.exist');
        });
    ...
    it(
        '1) operator4_fr, disconnected from ENTITY4_FR, acknowledges the cards "message3" and "message4", and we check\n' +
    ...
    it('check entities ack for a card sent to entities different from entities of the user', function () {
        ...
        cy.get('#opfab-feed-light-card-cypress-entitiesAcksMessage4_ItalianEntityRecipients .fa-check').should(
            'not.exist'
        );
        ...
        cy.contains('#opfab-card-details-btn-ack', 'ACKNOWLEDGE AND CLOSE');
        card.acknowledge();
