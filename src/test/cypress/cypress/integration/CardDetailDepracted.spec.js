    ...
    describe('Check card detail deprecated calls to templateGateway', function () {
        ...
        it(`Check card detail`, function () {
            ...
            cy.get('#templateGateway-getAllEntities').contains(
                'entity[0]:id=ENTITY1_FR,name=Control Center FR North,description=Control Center FR North,parents=ENTITY_FR,labels=FR1 label'
            );
            cy.get('#templateGateway-getAllEntities').contains(
                'entity[1]:id=ENTITY2_FR,name=Control Center FR South,description=Control Center FR South,parents=ENTITY_FR,labels=undefined'
            );
            cy.get('#templateGateway-getAllEntities').contains(
                'entity[2]:id=ENTITY3_FR,name=Control Center FR East,description=Control Center FR East,parents=ENTITY_FR,labels=undefined'
            );
            cy.get('#templateGateway-getAllEntities').contains(
                'entity[3]:id=ENTITY4_FR,name=Control Center FR West,description=Control Center FR West,parents=ENTITY_FR,labels=undefined'
            );
            cy.get('#templateGateway-getAllEntities').contains(
                'entity[4]:id=ENTITY_FR,name=French Control Centers,description=French Control Centers,parents=undefined,labels=undefined'
            );
            cy.get('#templateGateway-getAllEntities').contains(
                'entity[6]:id=IT_SUPERVISOR_ENTITY,name=IT SUPERVISION CENTER,description=IT SUPERVISION CENTER,parents=EUROPEAN_SUPERVISION_CENTERS,labels=undefined'
            );
