...
describe('User Card ', function () {
    ...
    describe('Recipients dropdown option for parent entity should display child entities list', function () {
        it('Recipients dropdown option for parent entity should display child entities list', () => {
            ...
            cy.get('#opfab-recipients')
                .find('.vscomp-option-text')
                .eq(11)
                .contains(
                    '(Control Center FR East,Control Center FR North,Control Center FR South,Control Center FR West)'
                );
        });
    });
    ...
    describe('Recipients dropdown should not be displayed or restricted for some user cards', function () {
        ...
        it('Recipients should not be displayed in IT incident user card but set via template code', () => {
            ...
            cy.get('#opfab-entity-recipients')
                .find('.opfab-tooltip-content')
                .eq(0)
                .contains(
                    'Control Center FR East,Control Center FR North,Control Center FR South,Control Center FR West'
                );
    ...
    describe('Fields visibility', function () {
        ...
        it('Process select should be displayed even if there is only one process', () => {
            ...
            cy.get('#of-usercard-process-filter')
                .find('.vscomp-option-text')
                .eq(0)
                .should('contain', 'Process example ');
        });
    ...
    describe('Show automated response in preview for some user cards', function () {
        ...
        it('Show automated response in preview for Confirmation user card if user enabled to respond', () => {
            ...
            cy.get('of-light-card')
                .eq(0)
                .find('#opfab-feed-lightcard-hasChildCardFromCurrentUserEntity')
                .should('exist');
    ...
    describe('Send user card', function () {
        ...
        it('Send User card from operator1_fr', () => {
            ...
            cy.get('#message')
                .find('div')
                .eq(0)
                .type("Hello, that's a test message / Result is <OK> & work done is 100%");
            ...
            card.checkContainsText("Hello, that's a test message / Result is <OK> & work done is 100%");
        });
    ...
    describe('Edit user card', function () {
        ...
        it('Receive modified User card', () => {
            opfab.loginWithUser('operator2_fr');
            feed.openFirstCard();
            card.checkContainsText("Hello, that's a test message / Result is <OK> & work done is 100% (updated)");
            feed.checkSelectedCardHasTitle('Message');
            feed.checkSelectedCardHasSummary('Message received : Message received');
        });
    });
    ...
    describe('Send user card with operator4_fr, member of several entities', function () {
        ...
        it('Send User card from operator4_fr', () => {
            ...
            cy.get('#of-usercard-card-emitter-selector')
                .find('.vscomp-option-text')
                .eq(3)
                .should('contain.text', 'Control Center FR West');
        ...
        it('Disconnect operator4_fr from Control Center FR North and check card emitter when editing previous user card', () => {
            ...
            cy.get('#of-usercard-card-emitter-selector')
                .find('.vscomp-value')
                .should('have.text', 'Control Center FR East');
        ...
        it('Edit User card from operator4_fr with restricted list of emitters for a state', () => {
            ...
            feed.openFirstCard();
            card.checkContainsText('Process is in state');
    ...
    describe('Loading start date, end date and lttd from template', function () {
        it('Check dates are loaded from template for confirmation user card', () => {
            ...
            const expectedLttdDate =
                lttdDate.getFullYear() +
                '-' +
                (lttdDate.getMonth() + 1).toString().padStart(2, '0') +
                '-' +
                lttdDate.getDate().toString().padStart(2, '0');
            ...
            cy.get('#opfab-usercard-startdate-choice-input')
                .invoke('val')
                .then((text) => {
                    expect(expectedStartDateTime).to.equal(text);
                });

            cy.get('#opfab-usercard-enddate-choice-input')
                .invoke('val')
                .then((text) => {
                    expect(expectedEndDateTime).to.equal(text);
                });

            cy.get('#opfab-usercard-lttd-choice-input')
                .invoke('val')
                .then((text) => {
    ...
    describe('Check entityRecipientsForInformation field', function () {
        it('Choose an entity recipient for information not included in entity recipients', () => {
            ...
            cy.get('#opfab-entity-recipients-for-information')
                .find('.opfab-activityarea-badge')
            ...
            cy.get('#opfab-entity-recipients-for-information')
                .find('.opfab-tooltip-content')
                .eq(0)
                .contains('IT SUPERVISION CENTER');
            ...
            cy.get('#opfab-card-acknowledged-footer')
                .find('span')
                .eq(1)
                .should('have.text', '\u00a0 Control Center FR North \u00a0')
        ...
        it('Check restricted list of entity recipients for information and values selected by default', () => {
            ...
            cy.get('#opfab-recipients-for-information')
                .find('.vscomp-option-text')
                .eq(0)
                .contains('Control Center FR East');
            cy.get('#opfab-recipients-for-information')
                .find('.vscomp-option-text')
                .eq(1)
                .contains('Control Center FR North');
            cy.get('#opfab-recipients-for-information')
                .find('.vscomp-option-text')
                .eq(2)
                .contains('Control Center FR South');
            cy.get('#opfab-recipients-for-information')
                .find('.vscomp-option-text')
                .eq(3)
                .contains('Control Center FR West');
            cy.get('#opfab-recipients-for-information')
                .find('.vscomp-option-text')
                .eq(4)
                .contains('French Control Centers');
            cy.get('#opfab-recipients-for-information')
                .find('.vscomp-option-text')
                .eq(5)
                .contains('IT SUPERVISION CENTER');
    ...
    describe('Check "create a copy" feature', function () {
        ...
        it(
            'Check fields service/process/state/severity/data/recipients/recipients for information are copied from the original ' +
    ...
    describe('Send card on behalf of a user', function () {
        it('Send card on behalf of a user', () => {
            ...
            card.checkContainsText('Hello, this is a test for sending a card on behalf of a user');
