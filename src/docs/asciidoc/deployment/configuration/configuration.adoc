...
The 'node-services.yml' configuration file contains parameters common to all the node services and a dedicated section for each service.
Node services can be configured with the following properties:

|===
|name|default|mandatory?|Description
|operatorfabric.servicesUrls.authToken||yes|Authentication service URL
|operatorfabric.servicesUrls.users||yes|Users service URL
|operatorfabric.servicesUrls.cardsPublication||yes|Cards publication service URL
|operatorfabric.servicesUrls.cardsConsultation||yes|Cards consultation service URL
|operatorfabric.rabbitmq.host||yes|RabbitMQ host address
|operatorfabric.rabbitmq.port||yes|RabbitMQ listen port
|operatorfabric.rabbitmq.username||yes|RabbitMQ username
|operatorfabric.rabbitmq.password||yes|RabbitMQ password
|operatorfabric.logConfig.logFolder||yes|Log file folder
|operatorfabric.logConfig.logFile||yes|Log file name
|operatorfabric.logConfig.logLevel||yes|Default log level
...
|operatorfabric.mongodb.database||yes|MongoDB database name
|operatorfabric.mongodb.uri||yes|MongoDB connection URI
|operatorfabric.cardsExternalDiffusion.adminPort||yes|Listen port
|operatorfabric.cardsExternalDiffusion.activeOnStartup||yes|Flag to start the notification service on startup
|operatorfabric.cardsExternalDiffusion.opfab.login||yes|user account used to call Opfab services
|operatorfabric.cardsExternalDiffusion.opfab.password||yes|user password to call Opfab services
|operatorfabric.cardsExternalDiffusion.defaultConfig.mailFrom||yes|Mail from address
|operatorfabric.cardsExternalDiffusion.defaultConfig.subjectPrefix||yes|Mail subject prefix
|operatorfabric.cardsExternalDiffusion.defaultConfig.bodyPrefix|||Mail body prefix
|operatorfabric.cardsExternalDiffusion.defaultConfig.windowInSecondsForCardSearch|||Max time interval used to query new cards (in seconds)
|operatorfabric.cardsExternalDiffusion.defaultConfig.secondsAfterPublicationToConsiderCardAsNotRead|||Time period after card publication before sending mail notification
|operatorfabric.cardsExternalDiffusion.defaultConfig.checkPeriodInSeconds|||Time interval between consecutive checks
|operatorfabric.cardsReminder.adminPort||yes|Cards reminder service Listen port
|operatorfabric.cardsReminder.activeOnStartup||yes|Flag to start the cards reminder service on startup
|operatorfabric.cardsReminder.checkPeriodInSeconds|||Cards remonder time interval between consecutive checks
|operatorfabric.supervisor.adminPort||yes|Cards reminder service Listen port
|operatorfabric.supervisor.activeOnStartup||yes|Flag to start the cards reminder service on startup
|operatorfabric.supervisor.defaultConfig.entitiesToSupervise|||List of entity id and related supervisor list ({ "id": "ENTITY1", "supervisors": ["ENTITY2"] })
|operatorfabric.supervisor.defaultConfig.disconnectedCardTemplate|||Card template used to send connections alerts
|operatorfabric.supervisor.defaultConfig.secondsBetweenConnectionChecks|||Connection check period
|operatorfabric.supervisor.defaultConfig.nbOfConsecutiveNotConnectedToSendFirstCard|||Number of consecutive failed checks before sending first alert card
|operatorfabric.supervisor.defaultConfig.nbOfConsecutiveNotConnectedToSendSecondCard|||Number of consecutive failed checks before sending second alert card
|operatorfabric.supervisor.defaultConfig.processesToSupervise|||List of processes and related states to monitor for missing acknowledgment ({ "process": "defaultProcess", "states": ["processState"] })
|operatorfabric.supervisor.defaultConfig.unackCardTemplate|||Card template used to send acknowledgment alerts
|operatorfabric.supervisor.defaultConfig.windowInSecondsForCardSearch|||Time period used to search for not acknowledged cards
|operatorfabric.supervisor.defaultConfig.secondsBetweenAcknowledgmentChecks|||Card acknowledgment check period
|operatorfabric.supervisor.defaultConfig.secondsAfterPublicationToConsiderCardAsNotAcknowledged|||Time period to wait after card publishing to consider a card as not acknowledged and send the alert
|===
