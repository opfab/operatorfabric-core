...
To enable Kafka you need to set the topic name for which the Kafka consumer can consume messages. You can do this be uncommenting the `kafka.consumer.group_id` in the `cards-publication.yml`
file or by setting the TOPIC in the `config/docker/kafka-env.properties` file :
[source, yaml]
----
  kafka:
    consumer:
