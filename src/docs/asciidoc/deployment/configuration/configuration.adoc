...
----
cd ./config/docker
./startOpfab.sh ~/config/local.env
----
...
|operatorfabric.heartbeat.checkIntervalInSeconds|10|no|Frequency at which the heartbeat from the ui to the server is checked.
|operatorfabric.heartbeat.delayInSecondsToConsiderUserDisconnected|100|no| After how many seconds without heartbeat, the user is considered disconnected
|===
...
The mail notification service can be configured with the following properties:
...
== Cards reminder service configuration

The cards reminder service configuration file is located in node-services/cards-reminder/conf folder.
The 'default-docker.json' file is an example that can be used to configure the environment by loading it into the docker container (erasing the existing one).
The service can be configured with the following properties:
