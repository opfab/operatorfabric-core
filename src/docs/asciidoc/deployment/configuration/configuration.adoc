...
A rabbitMQ component is provided in opfab, it has a default configuration.

 SECURITY WARNING : This default password is to be change if a rabbit port is exposed outside of docker network
...
|operatorfabric.cardsExternalDiffusion.activeOnStartup|true|false|Flag to start the notification service on startup
