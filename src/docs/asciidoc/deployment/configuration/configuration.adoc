...
Configuration is centralized in the config directory, the  sub-directory dev is specific for development environnement and the sub-directory docker is a specific configuration to use in a full docker environnement.
...
Each business service has a specific yaml configuration file. It contains a least the name of the service: 
...
It can contain as well references to other services, for example : 
[source, yml]
users:
  ribbon:
    listOfServers: users:8080

Examples of configuration of each business service can be found either in config/docker
...
The user service has theses specific properties :
