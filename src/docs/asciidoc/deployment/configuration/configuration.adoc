...
|operatorfabric.cardsExternalDiffusion.defaultConfig.dailyEmailTitle|'Cards received during the day'|no|Daily mail subject prefix
|operatorfabric.cardsExternalDiffusion.defaultConfig.hourToSendDailyEmail|7|no|Hour at which to send the daily recap email
|operatorfabric.cardsExternalDiffusion.defaultConfig.minuteToSendDailyEmail|30|no|Minute at which to send the daily recap email
