...
|supervisor.defaultConfig.secondsBetweenAcknowledmentChecks|||Card acknoledgement check period
|supervisor.defaultConfig.secondsAfterPublicationToConsiderCardAsNotAcknowleged|||Time period to wait after card publishing to consider a card as not acknowleged and send the alert
|===
...
The parameters in "supervisor.defaultConfig" section can be modified at runtime by sending an http POST request to the `/config` API of supervisor service with a JSON payloadcontaining the config parameters to be changed.
