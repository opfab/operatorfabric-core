...
[[jwt_mode]]
== Alternative way to manage groups, roles and entities

By default, `OperatorFabric` manages groups, roles and entities) through the user\'s collection in the database.
Another mode can be defined, the JWT mode. The groups, roles and entities) can come from the authentication token.
The administrator of the authentication service has to set what claims define the groups, roles and entities).
In the `Operator-Fabric` configuration, the opfab administrator has to set properties to retrieve those groups, roles and entities.
...
|operatorfabric.security.jwt.entitiesIdClaim||no| set the name of the entities field in the token
...
|operatorfabric.security.jwt.rolesIdClaim||no| set the name of the roles field in the token
|operatorfabric.security.jwt.gettingRolesFromToken||no| boolean indicating if you want the roles of the user to come from the token and not mongoDB (possible values : true/false)
|===
