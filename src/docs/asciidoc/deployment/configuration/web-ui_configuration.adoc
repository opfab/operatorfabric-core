...
The NGINX configuration is managed in the nginx.conf file:

* Under src/main/docker/nginx/nginx.conf if running in "dev" mode (using the dev-environment
docker-compose and run_all script)
* Under src/main/docker/deploy/nginx.conf if running in full docker mode using the deploy docker-compose

Please refer to the link:https://docs.nginx.com/[NGINX documentation] for more details.
