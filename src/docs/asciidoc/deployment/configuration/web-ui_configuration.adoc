...
|feed.geomap.darkMode|true|no|Active a filter for dark mode (if false dark mode and light have the same colors).
...
The layers and tiles of the map can be configured using the `feed.geomap.layers` property. This property accepts an array of objects, each representing a layer with a `type` property and additional configuration options depending on the layer type. The following layer types are supported:
    ...
    - `wmts`: WMTS layers. Requires `capabilitiesUrl`, `layer`, and `matrixSet`. You may also specify `useCredentialsForCapabilities` to `true` if capabilities retrieval requires authentication. 
...
"layers": [
                ...
                },
                {
                    ...
                    "matrixSet":"PM_0_19",
                    "useCredentialsForCapabilities": false
                },
                {
                    "type": "geojson",
