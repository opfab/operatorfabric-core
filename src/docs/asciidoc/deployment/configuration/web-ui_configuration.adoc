...
It is not necessary to declare `OperatorFabric` as application because it is added automaticaly with current release version and rank `0`. +
For example adding `Keycloak` appplication, with `'Keycloak'` as `name`, `1` as `rank` and  `'6.0.1'` as `version` should look like:
[source, json]
----
"about": {
    "keycloack": {
