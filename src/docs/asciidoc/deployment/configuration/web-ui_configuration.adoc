...
|settings.infos.hide.remoteLoggingEnabled|false|no|Control if we want to hide(true) or display(false or not specified) the checkbox to activate remote logging 
...
|settings.remoteLoggingEnabled|false|no| If set to true, some logs form the UI are send to the back and write in the log file of the cards-consultation service
...
|logo.height|40|no|The height of the logo (in px) (only taken into account if logo.base64 is set). The value cannot be more than 48px (if it is set to more than 48px, it will be ignore and set to 48px).
