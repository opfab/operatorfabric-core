...
|feed.geomap.darkMode|not defined|no| Controls **how/if** the night filter is applied. `false` = always light (no filter), `true` = always dark (apply filter), not defined = follows application theme / `dayNightMode`.
...
|feed.geomap.mapProjection|EPSG:3857|no| The map projection to use for displaying the map. Supported values: `EPSG:3857` (Web Mercator, default) and `EPSG:2154` (Lambert 93 for France). Initial coordinates can be provided in WGS84 (EPSG:4326) or in the target projection - the system automatically detects the format.
...
The map appearance is controlled by a single parameter:
...
==== Layer Types
    ...
    - `xyz`: XYZ tiles. Requires:
        ...
        ** `tileSize`: Optional integer (e.g., 256). Required for custom background layers

    - `wmts`: WMTS layers. Requires:
        ** `capabilitiesUrl`: URL to WMTS capabilities document (uses `credentials: 'include'` for domains in `credentialDomains`)
        ** `layer`: Layer identifier from capabilities
        ** `matrixSet`: Matrix set identifier from capabilities
    
    - `arcgis` or `esri`: ArcGIS REST Map Service layers. Requires:
        ** `url`: ArcGIS REST service endpoint URL (authentication tokens automatically added for domains in `credentialDomains`)
