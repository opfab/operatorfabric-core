...
If you are not accessing the server from localhost, there is a bug with authentication redirection. Your must use the following URL, replacing `SERVER_IP` by the IP address of your server:
...
To send the bundle you need to be authentified . To do that you need to get a token with the following command:
...
To modelize this process in OperatorFabric, we will use a "Process" with "States", we will modelize this in the `config.json` of the bundle:
