...
Let's send this card (for ENTITY1 and Dispatcher group with process/state not in user's perimeter) :
[source,JSON]
----
{
	"publisher" : "message-publisher",
...
Now let's send this card (for ENTITY1 and Dispatcher group with process/state in user's perimeter) :
[source,JSON]
----
{
	"publisher" : "message-publisher",
