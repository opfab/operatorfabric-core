...
+
```
./bin/prepare_release_version.sh
```
+
...
* Change the version from W.W.W.RELEASE (previous release) to X.X.X.RELEASE in the deploy and demo docker-compose file
+
...
==== Checking demonstration docker-compose files
...
. Use the ./bin/prepare_snapshot_version.sh script to automatically perform all the necessary changes:
+
```
./bin/prepare_snapshot_version.sh --version Y.Y.Y.SNAPSHOT
```
+
This will perform the following changes:
+
