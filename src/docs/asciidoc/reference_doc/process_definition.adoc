...
For question cards, you can choose to display the card header or not. This header contains the list of entities
that have already responded or not, and a countdown indicating the time remaining to respond, if necessary.
This setting is done using the `showDetailCardHeader` variable, in the definition of
the process state, in `config.json` file.
