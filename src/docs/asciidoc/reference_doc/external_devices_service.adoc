...
OperatorFabric automatically attempts to connect to configured and enabled (attribute isEnabled = true) external devices on start up. 
Besides, OperatorFabric regularly searches for disconnected devices, and tries to reconnect them if they are enabled.
Similarly, posting a new device configuration to the external devices service causes it to attempt to connect promptly.


In addition, if a notification is received by the external devices service that needs to be passed on to a device that is
