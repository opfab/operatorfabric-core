...
* Process requests from the UI (e.g. "play the sound for ALARM for user operator1") and translate them as requests to
...
Here is what happens when user operator1 receives a card with severity ALARM:
...
for operator1, how to connect to it and what signal "ALARM" translates to on this particular device.
