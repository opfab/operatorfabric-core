...
If the connected user is a member of `group1`, then he has a `Receive` right on process/state `process/myState`. So if the user is also a member of `ENTITY1_FR` then he will receive the card.
