...
== Recipients
When sending a user card, by default it is possibile to choose the recipients from all the available entities. To limit the list of available recipients it is possible to configure the field `recipientList` in `config.json`
