...
- "OnlyForEntitiesAllowedToEdit" : the entities acknowledgments are displayed for the members of the entities allowed to
edit the card
...
It is possible to set a recurrent reminder via the structure recurrence which define a regular event in the timespan structure. It is defined with the following fields : 
 ...
 - DaysOfWeek : a list of day of the week when the event arises. The day of week is a number with 1 being Monday and 7 being Sunday as defined in https://en.wikipedia.org/wiki/ISO_8601#Week_dates[the ISO Standard 8601 (weekday number) ]
