...
It is possible to use the component also as single value select. The advantage over a standard select is the possibility to benefit of the serching feature. To use the component as single select, initialize it with the `multiple` property set to false:  
...
Once the card is loaded, there may be some processing required before displaying it (see https://opfab.github.io/documentation/current/reference_doc/#_method_ontemplaterenderingcomplete[here]).
If this processing is long, it is possible to display a spinner when the card is being processed.

In order to do this, the card template can use the methods _templateGateway.displayLoadingSpinner()_ and _templateGateway.hideLoadingSpinner()_.
