...
To get a HTML representation from a Delta JSON string you can call the `opfab.richTextEditor.getHtml(delta)` method. For example:

....
<span id="richMessage"></span>

<script>

  document.getElementById("richMessage").innerHTML = opfab.richTextEditor.getHtml("{{card.data.richMessage}}");
  ...
</script>
