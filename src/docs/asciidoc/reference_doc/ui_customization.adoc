...
* `showOnlyForRoles` : List of roles for which this menu should be visible (array, optional)
...
** If `showOnlyForGroups` and `showOnlyForRoles` are not present, null or an empty array : the menu is visible for all users.
** If `showOnlyForGroups` or `showOnlyForGroups` is present and with a non-empty array as a value: menu is visible only for users from the listed groups or roles.
...
**
- `showOnlyForGroups`: Defines the list of user groups entitled to see the menu entry, if not defined or empty it will be visible to every user (unless `showOnlyForRoles` is defined and not empty).
- `showOnlyForRoles`: Defines the list of user roles entitled to see the menu entry, if not defined or empty it will be visible to every user (unless `showOnlyForGroups` is defined and not empty).
