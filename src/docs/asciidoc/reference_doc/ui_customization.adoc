...
For `HTML` field type, sorting has been deactivated because the HTML tags may falsify the order. Filtering is enabled but may also be impacted. The HTML tags need to be injected by the `getValue` method of the column.
