...
* Use `!!myVariable` to test if a variable is undefined or null except for number where zero will return false ( !!0 is equal to false). It is not mandatory for boolean. 
...
* Do not store Moment objects in ngrx store (see for example : https://github.com/ngrx/platform/issues/2690)
...
* https://lists.lfenergy.org/g/opfab-dev[OperatorFabric Developers] for project development discussions (not really active, we might abandon it in favour of GitHub Discussions)
