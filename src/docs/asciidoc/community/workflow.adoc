...
[IMPORTANT]
====
On this screen, by default the base branch for the PR is `master` because it is the repository's default
branch. However, feature branches are meant to be merged back into `develop`, not `master`, so you should select
`develop` in the base branch dropdown list.
...
[[review_checklist]]
include::review_checklist.adoc[leveloffset=+3]
