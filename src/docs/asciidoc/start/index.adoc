...
----
Launch docker-compose up in the server directory. You need to wait for all the services to start, it is done when no more logs are written on the output.
...
If you are not accessing the server from localhost, there is a bug with authentification redirection. Your must use the following URL ,replacing SERVER_IP by the IP adress of your server :
...
The way the card is display in the UI is defined via a Bundle containg templates and process description .
...
The bundle is provide in the bundle directory of example3. It contains a new version of the bundle used in example1 . 
...
Add the token to the sendBundle.sh script :
...
To send the card use the provide script in example4 directory 
