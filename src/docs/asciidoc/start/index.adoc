...
----
Launch docker-compose up in the server directory. You need to wait for all the services to start (it usally takes one minute to start),it is done when no more logs are written on the output(It could continue to log  but slowly).
