...
using a GPG key.
* the https://vanniktech.github.io/gradle-maven-publish-plugin/central/[Vanniktech Maven Publish] Gradle plugin to publish to the Maven Central Portal
...
For the publication to the staging repository (Central Portal) to work, you need to generate a user token on the https://central.sonatype.com/account[central interface] 
and set the credentials in your gradle.properties file:

----
mavenCentralUsername=SONATYPE_JIRA_USERNAME
mavenCentralPassword=SONATYPE_JIRA_PASSWORD
----
