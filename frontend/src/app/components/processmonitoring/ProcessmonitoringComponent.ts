...
import {ProcessMonitoringView} from 'app/components/processmonitoring/view/ProcessMonitoringView';
...
import {MonitoringMapComponent} from './components/map/MapComponent';
import {
...
export class ProcessMonitoringComponent implements OnDestroy, OnInit, AfterViewInit {
    ...
    onTableFilterChange(data) {
        ...
        Object.keys(data.filterModel).forEach((column) => {
            ...
            if (type === 'set') {
                if (data.filterModel[column].filter?.length > 0) {
                    this.columnFilters.push(
                        new Filter(colId, 'text', FilterMatchTypeEnum.IN, data.filterModel[column].filter)
                    );
                }
            } else {
                this.columnFilters.push(
