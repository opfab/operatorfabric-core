...
import {FieldType} from '@ofServices/customScreen/cardList/CardListScreenDefinition';
...
describe('CustomCardListView - Result Array - Acknowledgment', () => {
    ...
    it('isAcknowledgmentPossible should be true if user is allowed to acknowledge ', async () => {
        ...
        expect(dataArray).toEqual([{cardId: 'id1', testField: 'myProcess', isAcknowledgmentPossible: true}]);
    });
    it('isAcknowledgmentPossible should be false if user is not allowed to acknowledge', async () => {
        ...
        const dataArray = resultTable.getDataArrayFromCards([card], emptyChildCardsList);
        expect(dataArray).toEqual([{cardId: 'id1', testField: 'myProcess', isAcknowledgmentPossible: false}]);
    });
});
