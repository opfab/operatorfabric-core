...
describe('ActivityAreaView', () => {
    ...
    describe('Activity Area View - entity lines', () => {
        ...
        it('GIVEN a user WHEN he is member of entity with no parents or no parents with the correct role THEN activityArea has 2 lines with no title', async () => {
            ...
            expect(activityAreaPage.activityAreaClusters[0].lines[0].entityId).toEqual(
                'ENTITY_WITH_NO_CLUSTERING_PARENT'
            );
        ...
        it('GIVEN a user member of an entity with two parents WHEN the second parent has no ACTIVITY_AREA_GROUP role THEN the entity appears only under the first parent group', async () => {
            ...
            expect(activityAreaPage.activityAreaClusters[0].lines[0].entityName).toEqual(
                'ENTITY_WITH_TWO_PARENTS_NAME'
            );
        });
    });
    describe('Activity Area View - entity connection status and connected users', () => {
        it('GIVEN hideConnectedUsers is true WHEN activity area is initialized THEN isConnectedUsersVisible is false and connected users is empty', async () => {
            await mockUserConfig(['ENTITY1'], []);
            await loadWebUIConf({
                activityArea: {
                    hideConnectedUsers: true
                }
            });
            initActivityAreaView();
            const activityAreaPage = await firstValueFrom(activityAreaView.getActivityAreaPage());
            expect(activityAreaPage.isConnectedUsersVisible).toBeFalse();
        });
        ...
        it('GIVEN a user member of entity1 and entity2 WHEN entity1 has another user currently connected THEN entity1 line contains the other user login in the activityAreaView ', async () => {
            ...
            usersServerMock.setResponseForConnectedUsers(
                new ServerResponse(connectedUsers, ServerResponseStatus.OK, null)
            );
        ...
        it('GIVEN hideConnectedUsers is true WHEN users are connected THEN connected users is empty ', async () => {
            await mockUserConfig(['ENTITY1', 'ENTITY2'], []);
            await loadWebUIConf({
                activityArea: {
                    hideConnectedUsers: true
    ...
    describe('Activity Area View - save activity area', () => {
    ...
    describe('Activity Area View - checkboxes interactions', () => {
    ...
    describe('Activity Area View - regularly update connected users', () => {
