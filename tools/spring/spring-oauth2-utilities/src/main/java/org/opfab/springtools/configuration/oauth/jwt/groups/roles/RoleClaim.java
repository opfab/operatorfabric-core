 ...
 * Must contain the field path, which defines where will be the key in the JWT
 * structure.
 */
public abstract class RoleClaim {

    private static final Logger LOGGER = Logger.getLogger(RoleClaim.class.getName());
    ...
    protected RoleClaim(String path) {
        this.path = path;
    }
     ...
     * 3) if null, no role will be mapped
     * otherwise get a list of roles through a computation (defined by the child
     * class)
     *
     * @param jwt
     ...
     * if not, no role mapped
     * otherwise, get the list through the computation depends on the node format
     *
     * @param jsonNodeRoot
    ...
    /**
     * Get the list of roles from the JSON node that contains the structure of role
     * The computation will be dealt with by the child class
     *
