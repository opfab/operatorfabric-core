...
There are two mecanism of i18n. The first one is done using l10n files which are located in the `i18n` folder, the second one throuhougt l10n name folder nested in the `template` folder or in the `media` folder.
...
....
{
	"third-name-in-menu-bar":"Bundle de test",
		...
		"process":{
			...
			"summary":"Courte déscription de la carte",
			"detail":{
				"tab":{
