...
Operator Fabric Time Business service is a Spring MVC application
...
|operatorfabric.time.default|null|no| Forced time at startup in millis from epoch
...
```
include::../../../../../infra/config/src/main/docker/volume/dev-configurations/time.yml[]
```

=== Sample docker image configuration

```
include::../../../../../infra/config/src/main/docker/volume/docker-configurations/time.yml[]
```
