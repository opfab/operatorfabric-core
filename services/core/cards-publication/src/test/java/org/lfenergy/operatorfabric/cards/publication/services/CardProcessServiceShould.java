...
class CardProcessServiceShould {
    ...
    void processAddUserAcknowledgement() {
        ...
        UserAckOperationResult res = cardProcessingService.processUserAcknowledgement(Mono.just(cardUid), user).block();
    ...
    void processAddTraceAcknowledgement() {
        ...
        user.setLogin("aaa");
        UserAckOperationResult res = cardProcessingService.processUserAcknowledgement(Mono.just(cardUid), user).block();
        UserActionTraceData trace= traceRepository.findByCardUidAndActionAndUserName(cardUid,"Acknowledgment","aaa").block();
