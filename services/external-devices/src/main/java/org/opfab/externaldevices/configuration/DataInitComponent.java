...
/**
 * This component reads external devices configuration from a properties file
 * and updates/creates them as appropriate.
 */
@Component
public class DataInitComponent {

    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(DataInitComponent.class);
    ...
    /**
     * Initialize signal mappings using provided collection (if null, log debug
     * message).
     */
    ...
    /**
     * Initialize device configurations using provided collection (if null, log
     * debug message).
     */
    ...
    /**
     * Initialize user configurations using provided collection (if null, log debug
     * message).
     */
    ...
    /**
     * Inserts device configuration unless a configuration already exists with the
     * same id. In this case log it and
    ...
    private void safeInsertDeviceConfiguration(DeviceConfiguration deviceConfigurationData) {
        try {
            ...
            if (Boolean.TRUE.equals(deviceConfigurationData.isEnabled))
                enableDevice(deviceConfigurationData.id);
    ...
    /**
     * Inserts signal mapping unless a mapping already exists with the same id. In
     * this case log it and
