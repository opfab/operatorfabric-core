...
public class ConfigService {
    ...
    public List<ResolvedConfiguration> getResolvedConfigurationList(String opFabSignalKey, String userLogin)
    ...
    public DeviceConfiguration retrieveDeviceConfiguration(String deviceId)
            throws ExternalDeviceConfigurationException, UnknownExternalDeviceException {
        if (deviceId == null || deviceId.isEmpty()) {
            throw new ExternalDeviceConfigurationException(
                    String.format(CANNOT_RETRIEVE_FOR_NULL_OR_EMPTY_ID, DEVICE_CONFIG));
        } else {
            Optional<DeviceConfiguration> deviceConfiguration = deviceConfigurationRepository.findById(deviceId);
            if (deviceConfiguration.isPresent()) {
                DeviceConfiguration retrievedDeviceConfig = deviceConfiguration.get();
                ...
                throw new UnknownExternalDeviceException(
                        String.format(CONFIGURATION_NOT_FOUND, DEVICE_CONFIG, deviceId));
            }
        }
    }
    ...
    public UserConfiguration retrieveUserConfiguration(String userLogin) throws ExternalDeviceConfigurationException {
        if (userLogin == null || userLogin.isEmpty()) {
            throw new ExternalDeviceConfigurationException(
                    String.format(CANNOT_RETRIEVE_FOR_NULL_OR_EMPTY_ID, USER_CONFIG));
        } else {
            ...
            if (userConfiguration.isPresent()) {
                ...
                throw new ExternalDeviceConfigurationException(
                        String.format(CONFIGURATION_NOT_FOUND, USER_CONFIG, userLogin));
            }
        }
    }
    ...
    public SignalMapping retrieveSignalMapping(String signalMappingId) throws ExternalDeviceConfigurationException {
        if (signalMappingId == null || signalMappingId.isEmpty()) {
            throw new ExternalDeviceConfigurationException(
                    String.format(CANNOT_RETRIEVE_FOR_NULL_OR_EMPTY_ID, SIGNAL_CONFIG));
        }
        ...
        if (signalMapping.isPresent()) {
            ...
            throw new ExternalDeviceConfigurationException(
                    String.format(CONFIGURATION_NOT_FOUND, SIGNAL_CONFIG, signalMappingId));
        }

    }

    public void deleteDeviceConfiguration(String deviceId)
            throws ExternalDeviceConfigurationException, UnknownExternalDeviceException {
    ...
    public void deleteSignalMapping(String signalMappingId) throws ExternalDeviceConfigurationException {
        ...
        List<DeviceConfiguration> foundDeviceConfigurations = deviceConfigurationRepository
                .findBySignalMappingId(signalMappingId);
        if (foundDeviceConfigurations != null) {
            for (DeviceConfiguration deviceConfigurationData : foundDeviceConfigurations) {
                ...
                log.info(NULL_AFTER_DELETE, signalMappingId, "signalMapping", DEVICE_CONFIG,
                        deviceConfigurationData.id);
            }
    ...
    public void deleteUserConfiguration(String userLogin, Optional<Jwt> token)
            throws ExternalDeviceConfigurationException {
    ...
    private int computeSignalId(SignalMapping signalMapping, String opFabSignalKey)
            throws ExternalDeviceConfigurationException {
        if (signalMapping.supportedSignals.containsKey(opFabSignalKey)) {
            return signalMapping.supportedSignals.get(opFabSignalKey);
        } else {
            throw new ExternalDeviceConfigurationException(
                    String.format(UNSUPPORTED_SIGNAL, opFabSignalKey, signalMapping.id));
        }
    }
