...
Set the two objects with the data prepared and their booleans to configures timeline.
...
Finally, to add or remove a domain, change the corresponding letter for doaine name in __web-ui.json__ file:
```
	feed:
    subscription:
      timeout: 600000
...
Defined domaines are: J, TR, 7D, W, M and Y.