...
|operatorfabric.logo.height||no|The height of the logo (px)
|operatorfabric.logo.width||no|The width of the logo (px)
|operatorfabric.logo.limitSize||no|True by default. If it is true, the height limit is 64px and the width limit is 200px, it means that if the height is over than 64px, it will be set to 64px, if the width is over than 200px, it is set to 200px. If it is false, no limit restriction for the height and the width. 
|===
