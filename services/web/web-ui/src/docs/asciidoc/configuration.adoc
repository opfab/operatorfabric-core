...
|operatorfabric.security.logout-url||yes|The keycloak logout URL. In most cases, this parameter is a composition of your keycloak instance and the _auth_ keyword (ex: https://www.keycloakurl.com/auth), but we also support domains without _auth_ (ex: https://www.keycloakurl.com/customPath)
