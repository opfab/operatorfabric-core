...
public class CardRoutingUtilities {
    ...
    public static String createDeleteCardMessageForUserNotRecipient(JSONObject cardOperation,String userLogin) {
    ...
    public static boolean checkIfUserMustBeNotifiedForThisProcessState(String process, String state, CurrentUserWithPerimeters currentUserWithPerimeters) {
    ...
    private static Map<String, RightsEnum> loadUserRightsPerProcessAndState(CurrentUserWithPerimeters currentUserWithPerimeters) {
    ...
    private static boolean isReceiveRightsForProcessAndState(String processId, String stateId, Map<String, RightsEnum> userRightsPerProcessAndState) {
    ...
    public static boolean checkIfUserMustReceiveTheCard(JSONObject cardOperation, CurrentUserWithPerimeters currentUserWithPerimeters) {
        ...
        log.debug("Check if user {} shall receive card {} for processStateKey {}", currentUserWithPerimeters.getUserData().getLogin(), idCard, processStateKey);
        ...
        if (checkInCaseOfCardSentToUserDirectly(userRecipientsIdsArray,currentUserWithPerimeters.getUserData().getLogin())) {  // user only
            log.debug("User {} is in user recipients and shall receive card {}", currentUserWithPerimeters.getUserData().getLogin(), idCard);
            return true;
        }

        if (checkInCaseOfCardSentToGroupOrEntityOrBoth(userGroups, groupRecipientsIdsArray, userEntities, entityRecipientsIdsArray)) {
            log.debug("User {} is member of a group or/and entity that shall receive card {}", currentUserWithPerimeters.getUserData().getLogin(), idCard);
            return true;
        }
        return false;
    }
