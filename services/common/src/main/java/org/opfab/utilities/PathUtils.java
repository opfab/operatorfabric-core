...
public class PathUtils {
    ...
    private static void throwExceptionIfPathIsOutsideOfApplicationBasePath(Path path) throws IOException {
        if (isPathOutsideOfApplicationBasePath(path))
            throw new IOException("Path " + path.toAbsolutePath().normalize().toString()
                    + " is not in application base path " + applicationBasePath);
    }
        ...
        try (BufferedInputStream bis = new BufferedInputStream(is);
            ...
            while ((entry = tis.getNextEntry()) != null) {
                ...
                if (entry.isDirectory()) {
                    ...
                    // there is no security issue here as the path has been checked before via
                    // throwExceptionIfPathIsOutsideOfApplicationBasePath
    ...
    @SuppressWarnings("javasecurity:S6096") // there is no security issue here as the path has been checked before via
                                            // throwExceptionIfPathIsOutsideOfApplicationBasePath
...
class CopyDir extends SimpleFileVisitor<Path> {
    ...
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(CopyDir.class);
...
class DeleteDir extends SimpleFileVisitor<Path> {
    ...
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(DeleteDir.class);
