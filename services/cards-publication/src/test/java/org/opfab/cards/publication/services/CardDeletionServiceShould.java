...
class CardDeletionServiceShould {
    ...
    void GIVEN_a_parent_card_and_a_child_card_WHEN_deleting_the_parent_card_THEN_child_card_is_deleted() {
        ...
        cardDeletionService.deleteCardByIdWithUser(card.id, Optional.empty(), token);
    ...
    void GIVEN_5_cards_with_two_cards_expiration_date_in_the_past_WHEN_delete_cards_by_expirationDate_set_to_now_THEN_2_cards_are_deleted() {
        ...
        cards.get(0).expirationDate = null;
        cards.get(1).expirationDate = null;
        cards.get(2).expirationDate = ref.plusSeconds(10000);
        cards.get(3).startDate = ref.minusSeconds(20000);
        cards.get(3).expirationDate = ref.minusSeconds(10000);
        cards.get(4).startDate = ref.minusSeconds(20000);
        cards.get(4).expirationDate = ref.minusSeconds(10000);
        cards.forEach(card -> cardProcessingService.processCard(card));
