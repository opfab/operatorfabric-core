...
export class SingleFilterComponent implements OnInit, OnChanges {
    ...
    ngOnInit() {
        ...
        if (this.sortValues) this.sortPeparedList();
    ...
    private translateItem(item: string, parameters?): string {
        let translated = item;
        if (!!parameters) this.translateService.get(item, parameters).subscribe((result) => (translated = result));
        else this.translateService.get(item, null).subscribe((result) => (translated = result));
        return translated;
    }
