...
export class MonitoringMapComponent implements OnInit, OnChanges, OnDestroy, AfterViewChecked {
        ...
        if (this.map) {
            ...
            this.vectorLayer = new VectorLayer({
                ...
                style: function (feature) {
                    const severity: Severity = feature.get('lightCard').severity;
                    const geoType: string = feature.getGeometry().getType();
                    return MonitoringMapComponent.getOpenLayersStyle(geoType, severity);
                }
            });
    ...
    private static getOpenLayersStyle(type: string, severity: Severity, highlight = false): Style {
        switch (type) {
            case 'Point':
                return MonitoringMapComponent.pointStyle(severity, highlight);
            case 'Polygon':
                return MonitoringMapComponent.polygonStyle(severity, highlight);
            default:
    ...
    private static pointStyle(severity: Severity, highlight: boolean) {
        ...
        return new Style({
            image: new Circle({
                ...
                fill: new Fill({
                    color: MonitoringMapComponent.severityToColorMap(0.8)[severity]
                }),
                stroke: new Stroke({
    ...
    private static polygonStyle(severity: Severity, highlight: boolean) {
        ...
        return new Style({
            ...
            fill: new Fill({
                color: MonitoringMapComponent.severityToColorMap(fillOpacity)[severity] // 'rgba(0, 0, 255, 0.1)'
            })
        });
    }
