...
export class TaskCardTemplateView {
    ...
    public getSpecificCardInformation(taskTitle, taskDescription, freq, durationInMinutes, minutesForReminder, byweekday, bymonth, bysetpos, bymonthday, time) {
    ...
    public getTaskDescription() {
        ...
        if (opfab.currentUserCard.getEditionMode() !== 'CREATE') {
            ...
            if (taskDescriptionField) {
                taskDescription = opfab.utils.convertSpacesAndNewLinesInHTML(opfab.utils.escapeHtml(taskDescriptionField));
            }
        }
